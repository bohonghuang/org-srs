...
(defun org-srs-review-cache-sub-predicate-p (pred1 pred2)
  (pcase pred2
    ...
    (_ (when (equal pred1 pred2) 'equal))))
      ...
      (cl-destructuring-bind (desc) args
        (cl-etypecase desc
          ((or list symbol)
           (cl-function
            (lambda (&rest args)
              (let ((current-item (or args (cl-multiple-value-list (org-srs-item-at-point)))))
                (cl-assert (and (org-srs-reviewing-p) (org-srs-review-cache-query-p)))
                (cl-loop with cache = (org-srs-review-cache)
                         ...
                         if (cl-loop for item in items thereis (org-srs-review-cache-item-equal item current-item)) return t
                         else if (eq sub-predicate-p 'equal) return nil
                         finally (cl-return (cl-find current-item (org-srs-query desc (org-srs-review-cache-source cache))
